...
- 当前基准标签：`baseline-2026-07-26-p2`
- 上一基准标签：`baseline-2026-07-26-p1`
- 稳定维护分支：`codex/baseline-stable`
- 算法与架构优化分支：`main`
- 当前基准更新日期：2026-07-26

当前基准以成员改动整理版为基础，包含 2026-07-25 的新视觉模型、类别显示和实机参数；包含实时视觉停止与栏目焦点补丁；并统一桌面按钮的可用/禁用状态以及主要任务蓝色背景。此前所有基准标签均继续保留为历史回退点。
...
- 桌面按钮具有统一、清晰的可用/禁用样式，主要任务按钮统一为蓝底白字。
...
```powershell
git switch --detach baseline-2026-07-26-p2
```

查看包含视觉停止补丁、尚未包含按钮样式统一的基准：

```powershell
git switch --detach baseline-2026-07-26-p1
```

查看成员改动首次整理完成、尚未包含视觉停止补丁的基准：
