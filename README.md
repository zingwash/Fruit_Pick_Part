...
> - 近端采摘（按 `S` 键）：通过 near bbox 精定位；采摘点取葡萄串框上边中点 `top_center`，并向上偏移 `StemOffsetAboveCorePointM`（默认 2cm）；识别失败时优先做一次新的自动 far 识别，失败则使用 `context` 中保存的远端 far `top_center` 作为回退采摘点。
> - 近端采摘 far 校验（在完整循环中生效）：若 near 识别成功，会用远端靠近阶段保存的 far `top_center` 做偏差校验；若 near `top_center` 与 far `top_center` 距离 **> 0.5cm**，判定 near 失效，优先使用新的自动 far 识别，失败则回退远端 far `top_center`。
...
> - **远端靠近分阶段运动**：`FarApproachProfile` 支持 `UseStagedToolXyThenToolZ`（默认 `true`）。启用后，远端靠近先执行工具 X/Y 阶段（保持当前工具 Z 不变），再执行工具 Z 阶段（沿工具 Z 直线移动到目标），避免远距靠近时一开始就斜向插向葡萄串。
...
> - **远端靠近最小安全距离**：`FarApproachProfile` 支持 `MinTcpToTargetDistanceM`（默认 `0.05m`）。计算出的目标 TCP 若离 far `top_center` 小于该值，会自动沿远离目标方向回退，保证机械臂末端与葡萄串中心至少保持该距离。
