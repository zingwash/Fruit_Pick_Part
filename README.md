...
>
> 开始算法与架构优化前的稳定版本见 [《稳定基准版本说明》](BASELINE.md)。
...
### 2026-07-26：统一桌面按钮可用性样式与主要任务蓝色背景

- 所有通过桌面按钮工厂创建的普通按钮统一改用 `ThemeButton`，可点击时为白底深色文字，不可点击时为明显灰底浅灰文字。
- 禁用按钮使用普通箭头鼠标，可用按钮使用手型鼠标，进一步降低状态误判。
- 单次自动采摘、连续自动采摘、固定点任务、确认执行和轨迹批准等主要操作统一为蓝底白字，不再使用绿色按钮背景。
- 绿色继续用于“成功”“已连接”等状态文字；红色继续用于软件停止等危险操作。

### 2026-07-26：修复复用视觉进程后实时检测无法停止、无法切换栏目

- 修复自动任务等流程已经启动过 Python worker 后，再开启 Far/Near 实时检测时“停止当前视觉”按钮一直不可用的问题。
- 手动实时检测进入运行状态后会立即重新计算按钮权限，不再依赖异步更新的“检测中”文本状态。
- 启动实时检测时仍会主动打开一次视觉画面；后续预览帧只刷新图像，不再不断强制切回“视觉与夹爪”，因此检测期间可以正常浏览其他栏目。
- 自动任务的视觉截帧方式、机械臂运动逻辑、夹爪逻辑及所有实机参数均未改动。
